...
std::shared_ptr<CheatFunction> Cache = std::make_shared<CheatFunction>(2000, [] {
    ...
    if (base == 0) {
        ...
        if (TargetProcess.Init(ProcessName) ||
            TargetProcess.Init("DeadByDaylight-Win64-Shipping.exe") ||
            TargetProcess.Init("DeadByDaylight-EGS-Shipping.exe")) {
    ...
    uint64_t currentGWorld = TargetProcess.Read<uint64_t>(base + 0xd3f7958);
...
std::shared_ptr<CheatFunction> UpdateBonesFunc = std::make_shared<CheatFunction>(30, [] {
    if (!EngineInstance)
        return;
    auto handle = TargetProcess.CreateScatterHandle();
...
void RenderFrame()
{
    ...
    UpdateBonesFunc->Execute();

    RenderTarget->BeginDraw();
    RenderTarget->Clear(Colour(0, 0, 0, 255));
